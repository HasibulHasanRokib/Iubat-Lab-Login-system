...
const handleDelete=async()=>{
  ...
  return (
    ...
    <header className='flex justify-between items-center bg-white py-3 shadow-md px-16'>
     ...
     <section className=" py-4 px-6 mt-5">
     <form className="flex flex-col justify-center items-end" onSubmit={handleUpdate}>
      <table>
        ...
        <td>1</td>
          <td>{studentid}</td>
          <td ><input className="p-3 w-full outline-slate-300" type="text" id="fullname" onChange={handleChange} value={info.fullname}/></td>
          <td><input className=" w-full p-3 outline-slate-300"  type="email" id="email" onChange={handleChange} value={info.email} /></td>
          <td>{phone}</td>
          <td>{lastLogin[0].timestamp}</td>
          <td>{lastLogout[0]?.timestamp}</td>
          <td>{isLoggedIn===true?"Active":"Offline"}</td>
          <td>
            <select id="isBanned" value={info.isBanned}  className={`p-3 border-2 shadow-sm ${isBanned===true?"text-red-500":"text-green-500"} w-full outline-slate-300 font-bold`} onChange={handleChange}>
